...
export function DashboardPage(): HTMLElement {
    ...
    // --- Section pour les demandes d'amis (inchangée) ---
    ...
    // Contenu HTML du tableau de bord principal (inchangé au début)
    contentWrapper.innerHTML = `
        ...
        <!-- Les sections Amis et Demandes d'amis seront ajoutées ici -->
    ...
    // --- Nouvelle fonction pour afficher la liste d'amis ---
    ...
    //         const avatar = friend.avatar_url || 'https://via.placeholder.com/40/007bff/ffffff?text=' + displayName.charAt(0).toUpperCase(); // Fallback avatar
    //         let statusIndicator = '';
    //         if (friend.status) {
    //             const statusColor = friend.status === 'online' ? 'bg-green-500' : (friend.status === 'in-game' ? 'bg-yellow-500' : 'bg-gray-400');
    //             statusIndicator = `<span class="inline-block w-3 h-3 ${statusColor} rounded-full mr-2" title="${friend.status}"></span>`;
    //         }
    //
    //         return `
    //         <li data-friend-id="${friend.id}" class="p-3 bg-white border border-gray-200 rounded-md shadow-sm flex justify-between items-center">
    //             <div class="flex items-center">
    //                 <img src="${avatar}" alt="${displayName}" class="w-10 h-10 rounded-full mr-3 object-cover">
    //                 <div>
    //                     ${statusIndicator}
    //                     <strong class="text-green-700">${displayName}</strong>
    //                     <span class="text-xs text-gray-500 block">(${friend.username})</span>
    //                 </div>
    //             </div>
    //             <div>
    //                 <!-- Actions possibles pour un ami (ex: voir profil, envoyer message, supprimer) -->
    //                 <button data-action="view-profile" data-user-id="${friend.id}" class="text-xs bg-blue-500 hover:bg-blue-600 text-white font-semibold py-1 px-2 rounded mr-1">Profil</button>
    //                 <button data-action="remove-friend" data-user-id="${friend.id}" class="text-xs bg-red-500 hover:bg-red-600 text-white font-semibold py-1 px-2 rounded">Supprimer</button>
    //             </div>
    //         </li>
    //     `;
    //     }).join('');
    ...
    // --- Nouvelle fonction pour afficher la liste d'amis ---
    function renderFriendsList(friends: Friend[]) { // Assurez-vous que l'interface Friend est à jour aussi
        friendsCountSpan.textContent = friends.length.toString();
        if (!friends.length) {
            friendsListEl.innerHTML = `<li class="text-gray-500 italic">Vous n'avez pas encore d'amis.</li>`;
            return;
        ...
        friendsListEl.innerHTML = friends.map(friend => {
            // Utiliser les propriétés exactes de l'objet 'friend' tel que retourné par l'API
    ...
    // Gestionnaires d'événements pour les actions sur les demandes d'amis (inchangé)
