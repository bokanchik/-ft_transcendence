...
export async function checkAndCreateNextRound(tournamentId: string): Promise<void> {
    const { tournament, matches } = await getTournamentById(tournamentId);

    // Si le tournoi est déjà marqué comme fini, on ne fait rien.
    if (!tournament || tournament.status === 'finished' || !matches || matches.length === 0) {
        return;
    }

    const lastRoundNumber = Math.max(...matches.map(m => m.round_number));
    const lastRoundMatches = matches.filter(m => m.round_number === lastRoundNumber);

    // On vérifie si TOUS les matchs du dernier round créé ont un gagnant.
    const isRoundComplete = lastRoundMatches.every(m => m.winner_id !== null);

    if (isRoundComplete) {
        const winners = lastRoundMatches.map(m => m.winner_id);

        if (winners.length > 1) { // Il y a encore des gagnants à faire s'affronter
            if (winners.length % 2 !== 0) {
                // Ce cas ne devrait pas arriver dans un tournoi standard, mais c'est une sécurité
                console.error(`Incoherent number of winners (${winners.length}) for the next round in tournament ${tournamentId}.`);
                return;
            }

            const newRoundNumber = lastRoundNumber + 1;
            console.log(`Round ${lastRoundNumber} complete. Generating round ${newRoundNumber} for tournament ${tournamentId}.`);
            for (let i = 0; i < winners.length; i += 2) {
                const newMatchId = crypto.randomUUID();
                // On s'assure que les IDs sont bien des nombres
                const p1_id = Number(winners[i]);
                const p2_id = Number(winners[i + 1]);
                await addMatchToTournament(tournamentId, newMatchId, p1_id, p2_id, newRoundNumber);
            }
        } else if (winners.length === 1) { // C'est le grand gagnant !
            const finalWinnerId = Number(winners[0]);
            console.log(`Tournament ${tournamentId} finished. Winner is user ${finalWinnerId}.`);
            await execute(db, 'UPDATE tournaments SET winner_id = ?, status = ? WHERE id = ?', [finalWinnerId, 'finished', tournamentId]);
        }
    }
}