...
function initializeGame(gameMode: GameMode, ctx: CanvasRenderingContext2D, scoreDisplay: HTMLDivElement) {
    ...
    if (gameMode === 'remote' || gameMode === 'onlineTournament') {
        if (matchId && socket.connected) {
            socket.emit('playerInGameRoom', { matchId });
        } else {
            // Gérer le cas où le socket n'est pas connecté ou matchId est manquant
            console.error("Cannot signal playerInGameRoom: socket not connected or matchId missing.");
        }
    }
}
...
function clientSocketHandler(gameMode: GameMode, ctx: CanvasRenderingContext2D, scoreDisplay: HTMLDivElement) {
	...
	if (gameMode === 'onlineTournament' || gameMode === 'remote') {
		socket.on('opponentLeft', () => onGameOver());
	} else if (gameMode === 'local' || gameMode === 'tournament') {
		const matchId = sessionStorage.getItem('matchId');
...
async function onGameOver(finalState?: GameState) {
	...
	// Nettoyage des listeners et de la boucle de jeu côté client
    ...
    if (onlineTournamentId) {
        // Pour un tournoi en ligne, on ne montre pas l'écran de résultat individuel.
        // On retourne simplement à la page du tournoi, qui se mettra à jour via WebSocket.
        // On nettoie juste les infos du match actuel.
    ...
    if (gameMode === 'remote') {
        try {
            ...
            // Nettoyage complet après avoir affiché les résultats
            cleanupSocket(socket); 
            sessionStorage.clear();
        } catch (err) {
            console.error("Error on game over:", err);
