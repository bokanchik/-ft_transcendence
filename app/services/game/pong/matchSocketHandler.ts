...
// app/services/game/pong/matchSocketHandler.ts
...
async function disconnectionHandler(socket: Socket)  {
    ...
    socket.on('disconnect', async () => {
        ...
        if (gameSession) {
            ...
            // Logique différente pour tournoi ou match rapide
            const tournamentInfo = (socket as any).tournamentInfo;
            if (tournamentInfo && winnerId) {
                await handleMatchEnd(tournamentInfo.tournamentId, tournamentInfo.matchId, winnerId);
            } else if (winnerId) { // Pour les matchs rapides
                await setGameResult(gameSession.matchId, gameSession.state.score1, gameSession.state.score2, winnerId.toString(), 'forfeit');
                await Promise.all([
                    updateUserStatus(winnerId, UserOnlineStatus.ONLINE),
                    updateUserStatus(looserId, UserOnlineStatus.ONLINE)
                ]);
