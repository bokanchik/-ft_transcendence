...
const opponentsDetailsCache = {};
export async function MatchHistoryComponent(props) {
    ...
    try {
        ...
        loadingMessage.textContent = `${t('match.history.error')}: ${error.message}`;
...
function formatTimeAgo(date) {
    ...
    if (years > 0)
        return t(years === 1 ? 'time.ago.year' : 'time.ago.years', { count: years.toString() });
    const months = Math.floor(seconds / 2592000);
    if (months > 0)
        return t(months === 1 ? 'time.ago.month' : 'time.ago.months', { count: months.toString() });
    const days = Math.floor(seconds / 86400);
    if (days > 0)
        return t(days === 1 ? 'time.ago.day' : 'time.ago.days', { count: days.toString() });
    const hours = Math.floor(seconds / 3600);
    if (hours > 0)
        return t(hours === 1 ? 'time.ago.hour' : 'time.ago.hours', { count: hours.toString() });
    const minutes = Math.floor(seconds / 60);
    if (minutes > 0)
        return t(minutes === 1 ? 'time.ago.minute' : 'time.ago.minutes', { count: minutes.toString() });
