...
function createRequestListItem(user, actionsElement) {
...
export function FriendRequestsComponent(props) {
    ...
    section.append(createElement('div', { className: 'grid grid-cols-1 md:grid-cols-2 gap-6' }, [
        receivedContainer,
        sentContainer
    ]));
    return section;
}
function renderReceivedItems(requests, onAccept, onDecline) {
    if (!requests.length) {
...
function renderSentItems(requests, onCancel) {
    if (!requests.length) {
