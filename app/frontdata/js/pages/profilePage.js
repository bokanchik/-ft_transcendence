...
export async function ProfilePage(params) {
    const loggedInUser = getUserDataFromStorage();
    if (!loggedInUser) {
        navigateTo('/login');
    ...
    try {
        ...
        loadingProfileMsg.textContent = `${t('msg.error.user.loadingProfile')} : ${error.message}.`;
        loadingProfileMsg.classList.replace('text-gray-200', 'text-red-400');
    }
    return pageContainer;
}
function createProfileSidebar(profiledUser, loggedInUser) {
    const createSidebarItem = (label, value, isSensitive = false) => {
