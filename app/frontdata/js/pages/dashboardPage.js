...
export async function adjustFontSizeToFit(element, fontSizes = ['text-2xl', 'text-xl', 'text-lg', 'text-base', 'text-sm', 'text-xs'], truncateClass = 'truncate') {
...
export async function DashboardPage() {
    let currentUser = getUserDataFromStorage();
    if (!currentUser) {
        navigateTo('/login');
    ...
    function createSidebarItem(label, value) {
    ...
    function populateSidebar(user) {
        ...
        sidebar.append(profileHeader, createSidebarItem(t('user.username'), user.username), createSidebarItem(t('user.displayName'), user.display_name), createSidebarItem(t('user.email'), user.email), createSidebarItem(t('user.createdAt'), new Date(user.created_at)), createSidebarItem(t('user.wins'), user.wins), createSidebarItem(t('user.losses'), user.losses));
    }
    ...
    const handleSendFriendRequest = async (targetUserId) => {
        ...
        if (activeTabId === 'users' || activeTabId === 'pending')
    ...
    const handleAcceptFriendRequest = async (friendshipId) => {
        ...
        showToast(translateResultMessage(result.message), 'success');
        if (['users', 'pending', 'friends'].includes(activeTabId))
            await loadActiveTabContent();
    };
    ...
    async function loadUsersContent() {
        ...
        return UserList({
            users: usersData, friends: friendsData, sentRequests: sentRequestsData,
    ...
    async function loadFriendsContent() {
        ...
        return FriendsListComponent({
            ...
            onRemoveFriend: async (friendshipId) => {
                ...
                showToast(translateResultMessage(result.message), 'success');
                if (['friends', 'users'].includes(activeTabId))
                    await loadActiveTabContent();
            },
        });
    }
    ...
    async function loadMatchHistoryContent() {
